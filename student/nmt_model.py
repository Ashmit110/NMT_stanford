...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, _ = pad_packed_sequence(packed_enc_hiddens)  # Shape (src_len, b, h*2)
        enc_hiddens=enc_hiddens.permute(1, 0, 2)
        
        last_hidden = last_hidden.permute(1, 0, 2)  # Shape: (b, 2, h)
        init_decoder_hidden = self.h_projection(last_hidden.contiguous().view(last_hidden.size(0), -1))  # Shape: (b, h)
        
        
        last_cell = last_cell.permute(1, 0, 2)  # Shape: (b, 2, h)
        init_decoder_cell = self.c_projection(last_cell.contiguous().view(last_cell.size(0), -1))  # Shape: (b, h)

        # print(init_decoder_hidden.shape)
        # print(init_decoder_cell.shape)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Construct tensor Y of target sentences with shape (tgt_len, b, e)
        Y = self.model_embeddings.target(target_padded)  # Shape: (tgt_len, b, e)

        
         # Iterate over the target sequence time steps
        for Y_t in Y.split(1, dim=0):  # Y_t will have shape (1, b, e)
            Y_t = Y_t.squeeze(0)  # Shape: (b, e)

            # Concatenate Y_t with o_prev
            Ybar_t = torch.cat((Y_t, o_prev), dim=-1)  # Shape: (b, e + h)

            # Compute the decoder's next state and the new combined output o_t
            dec_state,combined_outputs,e_t=self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
            
            
            
            combined_outputs_list.append(combined_outputs)

            # Update o_prev to the new o_t
            o_prev = combined_outputs

        # Stack combined_outputs to a single tensor of shape (tgt_len, b, h)
        combined_outputs = torch.stack(combined_outputs_list)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Compute attention scores e_t
        # dec_hidden is (b, h), need to unsqueeze to (b, h, 1)
        dec_hidden_unsqueezed = dec_hidden.unsqueeze(2)  # Shape: (b, h, 1)
        
        # enc_hiddens_proj is (b, src_len, h), need to compute batched matrix multiplication
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden_unsqueezed).squeeze(2)  # Shape: (b, src_len)
        
        # The combined output is the decoder's hidden state
        ...
        ###         attention out ut vector, a_t.
        ###           - alpha_t is shape (b, src_len)
        ###           - enc_hiddens is shape (b, src_len, 2*h)      (b,h)
        ...
        alpha_t_unsqueezed = alpha_t.unsqueeze(1)  # Shape: (b, 1, src_len)
        a_t = torch.bmm(alpha_t_unsqueezed, enc_hiddens).squeeze(1)  # Shape: (b, 2*h)
        U_t=torch.cat([a_t,dec_hidden],dim=1)#shape (b,3*h)
